...
class Hawking:
    ...
    def preprocessing_and_data(self,tokenizer):
        ...
        df_pandas = df_pandas[df_pandas["topic;"].isin(['Thermodynamics','Special relativity','Dark matter','Black holes','Quantum mechanics','Plasma physics','Particle physics','String theory','Nuclear physics','Atomic physics','Quantum field theory','Gravitational waves','Electromagnetism','Chaos theory'])]
        ...
        A2_token_number = 510 - 28 - 150 - 50
        a1_series = tokenized_df["Answer_1"].swifter.apply(lambda x: tokenizer.decode(x["input_ids"][0][0:150],skip_special_tokens=True))
        q1_series = tokenized_df["Question_1"].swifter.apply(lambda x: tokenizer.decode(x["input_ids"][0][0:50],skip_special_tokens=True))
        a2_series = tokenized_df["Answer_2"].swifter.apply(lambda x: tokenizer.decode(x["input_ids"][0][0:A2_token_number],skip_special_tokens=True))

        a1_df = pd.DataFrame(a1_series)
        q1_df = pd.DataFrame(q1_series)
        a2_df = pd.DataFrame(a2_series)

        df_pandas_new = pd.concat([a1_df , q1_df, a2_df, df_pandas["Question_2"]], axis=1)
    ...
    def tokenize_function(self, example):
        start_prompt = 'Ask question based on answer:\n\n'
        instruction_prompt= start_prompt+"Answer\n\n{A1}\n\nQuestion\n\n{Q1}\nAnswer\n\n{A2}\n\nQuestion\n\n"
        prompt = [instruction_prompt.format(A1=a1, Q1=q1, A2=a2) for a1, q1, a2 in zip(example["Answer_1"], example["Question_1"], example["Answer_2"])]
    
        example['input_ids'] = self.tokenizer(prompt, padding="max_length", truncation=True, return_tensors="pt").input_ids.cuda(0)

        example['labels'] = self.tokenizer(example["Question_2"], padding="max_length", truncation=True, return_tensors="pt").input_ids.cuda(0)
    ...
    def zero_shot_make_prompt(example_indices):
        ...
        answer = dataset_df['Answer_1'][example_indices]
    #     answer = df_pandas["Answer_1"][example_indices]
        one_shot_prompt = instruction_prompt.format(A1=answer)
